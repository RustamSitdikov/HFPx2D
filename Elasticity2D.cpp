...
//    Function to get the normal and shear stress at a point on a surface (with
//    given normal and shear vector) induced by
// a linear DD segment of size h  (the linear DD segment is assumed to be the
// unit element along the x axis [-h/2,h/2]
...
//
//  st = stresses_kernel_dp1_dd(h, Ep, xe[0],
//                                 xe[1]); // columns sxxs, sxys, syys, syyn
//                                         // (knowing that we sxxn and sxyn are
//                                         // respectively equal to sxys and
//                                         syys
//                                         // )
//
...
//  sh1n = n1s1 * st(0, 1) + n1s2pn2s1 * st(0, 2) + n2s2 * st(0, 3); // normal
//  dd
//  // node 2
//  sh2s = n1s1 * st(1, 0) + n1s2pn2s1 * st(1, 1) + n2s2 * st(1, 2); // shear dd
//  sh2n = n1s1 * st(1, 1) + n1s2pn2s1 * st(1, 2) + n2s2 * st(1, 3); // normal
//  dd
//
//  // normal stress
//  // node 1
