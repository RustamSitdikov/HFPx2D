...
namespace hfp2d {
    ...
    il::StaticArray2D<double, 2, 4>
    stresses_kernel_linear_dd(const double h, const double Ep, const double x,
                           const double y) {
...
//  should have a check for h>0
        ...
        double xp = x *
                    overhalfh;   // change of variable to perform the computation for a unit segment [-1,1]
        double yp = y *
                    overhalfh;    // change of variable to perform the computation for a unit segment [-1,1]
...
// sxx xx stress, sxy xy stress, syy yy stress
        ...
        sxys1 = AtanAux1 -
                pow(r1, -1.) * pow(r2, -2.) * (pow(xpm1, 2.) * pow(xpp1, 3.) +
                                               2. * xp * (3. + xp) * xpp1 *
                                               pow(yp, 2.) +
                                               xpm1 * pow(yp, 4.));

        syys1 = pow(r1, -1.) * pow(r2, -2.) * (2 * xpm1 * yp * pow(xpp1, 2.) -
                                               2 * (1 + 3 * xp) * pow(yp, 3.));

        syyn1 = AtanAux1 -
                xpp1 * pow(r2, -2) * (pow(xpp1, 2) + 3 * pow(yp, 2)) +
                2 * xp * pow(yp, 2) *
        ...
        if (yp != 0.) // case of observation point on the unit sgement line
        {
            ...
            sxxs1 = AtanAuxxpm1 - AtanAuxxpp1 +
                    ...
                    (xpm1 * xpm2 * pow(xpp1, 2) +
                     (3 + dxpp3 * xp) * pow(yp, 2) + pow(yp, 4));//

            sxxs2 = -AtanAuxxpm1 + pow(r1, -2) * pow(r2, -1) *
                                   (AtanAuxxpp1 * r2 * pow(r1, 2) -
                                    2 * (2 + xp) * xpp1 * yp * pow(xpm1, 2) -
                                    2 * (3 + xp * (-3 + 2 * xp)) * pow(yp, 3) -
                                    2 * pow(yp, 5));
        } else {
            sxxs1 = ((il::pi) * (xpm1 / sqrt(pow(xpm1, 2.)) -
                                 xpp1 / sqrt(pow(xpp1, 2.)))) /
                    2.;    //could be simplified further as 2 or 0 depending on abs(xp)
            sxxs2 = ((il::pi) * (-(xpm1 / sqrt(pow(xpm1, 2.))) +
                                 xpp1 / sqrt(pow(xpp1, 2.)))) / 2.;
        }
...
//Material of Plane-strain Young's modulus Ep
    void normal_shear_stress_kernel_linear_dd(il::StaticArray2D<double, 2, 4> &St,
        ...
        st = stresses_kernel_linear_dd(h, Ep, xe[0],
                                    xe[1]);// columns sxxs, sxys, syys, syyn    (knowing that we sxxn and sxyn are respectively equal to sxys and syys )

        double sh1s, sn1s, sh1n, sn1n, sh2s, sn2s, sh2n, sn2n;
        //shear stress
        // node 1
        sh1s = n1s1 * st(0, 0) + n1s2pn2s1 * st(0, 1) +
               n2s2 * st(0, 2); //shear dd
        sh1n = n1s1 * st(0, 1) + n1s2pn2s1 * st(0, 2) +
               n2s2 * st(0, 3); // normal dd
        //node 2
        sh2s = n1s1 * st(1, 0) + n1s2pn2s1 * st(1, 1) +
               n2s2 * st(1, 2); //shear dd
        sh2n = n1s1 * st(1, 1) + n1s2pn2s1 * st(1, 2) +
               n2s2 * st(1, 3); // normal dd

        //normal stress
        // node 1
        sn1s = n1n1 * st(0, 0) + 2 * n1n2 * st(0, 1) +
               n2n2 * st(0, 2); // shear dd
        sn1n = n1n1 * st(0, 1) + 2 * n1n2 * st(0, 2) +
               n2n2 * st(0, 3); // normal dd
        //node 2
        sn2s = n1n1 * st(1, 0) + 2 * n1n2 * st(1, 1) +
               n2n2 * st(1, 2); // shear dd
        sn2n = n1n1 * st(1, 1) + 2 * n1n2 * st(1, 2) +
               n2n2 * st(1, 3); // normal dd
