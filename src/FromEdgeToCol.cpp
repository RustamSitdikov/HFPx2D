...
// The collocation points are located in the reference unit element at location
// {-1/sqrt(2) , +1/sqrt(2)}
// It returns a matrix (size 4 Nelts x Nelts+1) that, if multiplied by nodal
...
// dof_dim : dimension of the defree of freedom of pressure or DDs ?
// Dof :: dof handles of the elasticity unknowns (DP interpolation)
// Dofp :: dof handles for the scalar pressure with a CP interpolation
  ...
  for (il::int_t i = 0; i < Dof.size(0); ++i) {
    ...
    for (il::int_t j = 0; j < dof_dim; ++j) { // dof_dim
