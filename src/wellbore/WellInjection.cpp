...
void WellInjection::adjHFVolRate() {
  il::int_t n_hf = injection_rate_.size();
  il::Array<double> new_vol_rate{n_hf};
  for (il::int_t i = 0; i < n_hf; ++i) {
    new_vol_rate[i] =
        hf_p_drop_[i] /
        (coef_perf_[i] * injection_rate_[i] +
         coef_tort_[i] * std::pow(injection_rate_[i], beta_tort_[i] - 1.0));
  }
...
il::Array<double> WellInjection::adjHFVolRate(WellInjection &well_inj) {
  il::Array<double> &hf_vol_rate = well_inj.injection_rate_;
...
void WellInjection::setHFVolRate(il::Array<double> &new_vol_rate, il::io_t,
  ...
  for (il::int_t i = 0; i < new_vol_rate.size(); ++i) {
    if (!isnan(injection_rate_[i])) {
      diff[i] = new_vol_rate[i] - injection_rate_[i];
    } else {
