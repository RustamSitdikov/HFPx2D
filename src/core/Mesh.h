...
/// SETTER
    ...
    il::int_t
    numNodes() const { return nodes_.size(0); }
    il::int_t
    numElems() const { return connectivity_.size(0); }
    il::int_t
    interpOrd() const { return interpolation_order_; }
    il::int_t
    numFracs() const
    {
...
//    std::cout << "Value " << theValue << " +1 " << theValue+1 << std::endl;
    ...
    numConds() const
    {
        return (*std::max_element(condition_id_.begin(), condition_id_.end()))
            + 1;
    }
    il::int_t
    numDisplDofsPerElem() const { return dof_handle_displacement_.size(1); }
    il::int_t
    ...
    double
    X(il::int_t k) const { return nodes_(k, 0); }
    // Read the Y coordinate of a node
    double
    Y(il::int_t k) const { return nodes_(k, 1); }

    // Read a particular element of the node coordinates
    double
    node(il::int_t k, il::int_t i) const { return nodes_(k, i); }
    ...
    il::int_t
    dofDispl(il::int_t k,
             il::int_t i) const { return dof_handle_displacement_(k, i); }
    il::int_t
    fracID(il::int_t k) const { return fracture_id_[k]; }
    il::int_t
    matID(il::int_t k) const { return material_id_[k]; }
    il::int_t
    condID(il::int_t k) const { return condition_id_[k]; }
    ...
    il::int_t
    ncoor() const { return nodes_.size(0); };

    il::Array2D<double>
    coor() const { return nodes_; };
    ...
    eltsize(il::int_t e)
    {
        ...
        xdiff[0] =
            nodes_(connectivity_(e, 1), 0) - nodes_(connectivity_(e, 0), 0);
        xdiff[1] =
            nodes_(connectivity_(e, 1), 1) - nodes_(connectivity_(e, 0), 1);
