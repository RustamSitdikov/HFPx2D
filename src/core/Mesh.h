...
class Mesh {  // class for 1D wellMesh of 1D segment elements ?
 ...
 public:
  ...
  Mesh(const il::Array2D<double> &Coordinates,
       const il::Array2D<il::int_t> &Connectivity, const il::Array<il::int_t > &MatID,
       const il::Array<il::int_t > &FractureID, const il::int_t interpolationOrder) {
    // check validity of inputs
    ...
    for (il::int_t i = 0; i < tipnodes_.size(); i++) {
      ...
      nl = 0;
      if ((orderedTipElts[jf * 2] != -1)) {  // already a node stored
