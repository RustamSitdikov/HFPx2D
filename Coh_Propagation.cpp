...
//    Mesh take_submesh(il::int_t n1, il::int_t n2, Mesh mesh_total) {//we consider n1 and n2
//// represents for the element number
//        Mesh meshC;
//        il::Array2D<int> connC{n2 - n1 + 1, 2, 0};
//        take_submatrixint(connC, 0, n2 - n1, 0, 1,
//                          mesh_total.connectivity_);//here also, the connectivity begins from 0.1.2.3...
...
//                    il::Array<double> delta_w, int i, int j,
...
//        take_submatrix(colocationlseg, 0, 1, 0, 1,
//                       mesh_large.Coor);//the first line
//        take_submatrix(colocationrseg, mesh_large.nelts() - 1,
//                       mesh_large.nelts(), 0, 1, mesh_large.Coor);
//        //the last line
//
//        SegmentCharacteristic segl = get_segment_DD_characteristic(
//                colocationlseg, p);
//        SegmentCharacteristic segr = get_segment_DD_characteristic(
//                colocationrseg, p);
//
...
//        take_submatrixint(idC, 0, j - i, 0, 3,
//                          id);//here you need to check,because the number of degree of freedom always begins at 0.1.2.3...
//
//        double syyl1 = stressoutput(meshC, idC, p, material.Ep,
//                                    colocationl(0, 0),
//                                    colocationl(0, 1), widthB)[1];
//        double syyl2 = stressoutput(meshC, idC, p, material.Ep,
//                                    colocationl(1, 0),
//                                    colocationl(1, 1), widthB)[1];
//        double syyr1 = stressoutput(meshC, idC, p, material.Ep,
//                                    colocationr(0, 0),
//                                    colocationr(0, 1), widthB)[1];
//        double syyr2 = stressoutput(meshC, idC, p, material.Ep,
//                                    colocationr(1, 0),
//                                    colocationr(1, 1), widthB)[1];
//
//        if ((syyl1 >= (material.sigma_t + initial_condition.sigma0) or
//             syyl2 >= (material.sigma_t + initial_condition.sigma0)) and
//            i >= 1) {
//            --i;
//        };
//        if ((syyr1 >= (material.sigma_t + initial_condition.sigma0) or
//             syyr2 >= (material.sigma_t + initial_condition.sigma0)) and
//            j <= id.size(0)) {
...
//        int dof = 2 * (p + 1);
//        il::Array<double> fP{dof * (j - i + 1), 0.};
//        if (i < i0) {
//            for (int m = 1; m < dof * (i0 - i); m += p + 1) {
//                fP[m] = -pressure_f;
//            }
//        }
//        if (j > j0) {
//            for (il::int_t s = dof * (j0 - i + 1) + 1;
//                 s < dof * (j - i + 1); s += p + 1) {
//                fP[s] = -pressure_f;
//            }
//        }
...
//
//        int dof = 2 * (p + 1);
//        il::Array<double> fK{dof * (j - i + 1), 0.};
//        il::Array<double> vectem = il::dot(kmatC, widthB);
//        if (i < i0) {
//            for (int k = 0; k < dof * (i0 - i); ++k) {
//                fK[k] = -vectem[k];
//            }
//        }
//        if (j > j0) {
//            for (il::int_t s = dof * (j0 - i + 1); s < dof * (j - i + 1); ++s) {
//                fK[s] = -vectem[s];
//            }
//        }
//        return fK;
...
//
//
//    void construct_matrix(il::Array2D<double> &kmatC, il::Array<double> vwc,//here not sure whether there should be a reference for kmatC
...
//        il::Array<double> fP = former_pressure(pressure_f, i, j, i0, j0, p);
//        il::Array<double> fK = former_k(kmatC, i, j, i0, j0, widthB,
//                                        p);//added the 6th Februray
//
//
//        for (int q = 0; q < n; ++q) {
...
//        il::Array2D<double> kmat{4 * mesh_total.nelts(), 4 * mesh_total.nelts(),
//                                 0.};
...
//        for (il::int_t m = 0; m < 4 * j - 4 * i + 4; m += 2) {
//            F[m] = 0;
//        };
...
//                    il::Array<double> widthB,
//                    double pressure_f) {// i and j stands for the element number starting from 1
//        il::Array2D<double> kmat{id.size(0) * id.size(1),
//                                 id.size(0) * id.size(1), 0.};
//        il::Array2D<double> kmatnew{4 * (j - i + 1), 4 * (j - i + 1), 0.};
//        BasicAssembly(kmat, mesh_total, id, p, material.Ep);
//        take_submatrix(kmatnew, 4 * i - 4, 4 * j + 3 - 4, 4 * i - 4,
//                       4 * j + 3 - 4,
...
//        for (il::int_t m = 4 * i; m < 4 * j + 4; ++m) {
//            vwc[m - 4 * i] = vwc0[m];
//        }
//        il::Array<double> unitm{4 * (j - i + 1), 1.};
//        for (il::int_t s = 0; s < 4 * (j - i + 1); s += 2) {
//            unitm[s] = 0.;
//        }
...
//            il::Array<double> coh = cohesive_force(material, width_inm, i, j,
//                                                   i0, j0);
...
//                     Mesh mesh_total, il::Array2D<int> id, int p,
//                     Material material, Initial_condition initial_condition,
//                     il::int_t i0, il::int_t j0, int nstep, il::Status status) {
//        int n = mesh_total.nelts();
//        //double l;//crack length
//
//        il::Array<double> widthB;
//        initialwidth(widthB, mesh_total, id, p, material, initial_condition, i0,
//                     j0, status);
//        il::Array<double> width;
//        il::Array2D<double> widthlistinter{nstep, 4 * n,
...
//                plasticity_loop(delta_width, pressure_change, material,
//                                initial_condition,
//                                i, j, i0, j0, mesh_total, id, p, widthB,
//                                pressure);
//
//                il::Array<int> index{2, 0};
//                stress_criteria(index, mesh_total, id, material,
//                                initial_condition,
...
//                }
//                il::Array<double> delta_width_new{4 * (index[1] - index[0] + 1),
//                                                  0.};
//                for (int dwn = 0; dwn < widthB.size(); dwn += 1) {
//                    delta_width_new[4 * (i - index[0]) +
//                                    dwn] = delta_width[dwn];
//                }
